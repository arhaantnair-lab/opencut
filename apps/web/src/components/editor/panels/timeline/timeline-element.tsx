		...
		>
			...
			>
				...
				>
					...
					<TransitionsIndicator element={element} zoomLevel={zoomLevel} />
				</div>
			</div>
...
function TransitionsIndicator({
	...
	if (!element.transitions) return null;

	const { in: transitionIn, out: transitionOut } = element.transitions;

	return (
		<>
			{transitionIn && (
				<div
					className="absolute top-0 bottom-0 left-0 bg-white/20 border-r border-white/40 pointer-events-none"
					style={{
						width: `${timelineTimeToPixels({ time: transitionIn.duration, zoomLevel })}px`,
					}}
				/>
			)}
			{transitionOut && (
				<div
					className="absolute top-0 bottom-0 right-0 bg-white/20 border-l border-white/40 pointer-events-none"
					style={{
						width: `${timelineTimeToPixels({ time: transitionOut.duration, zoomLevel })}px`,
					}}
				/>
			)}
		</>
